...
bool fails(int result, int expected, const std::string& description) {
...
int main(int argc, char* argv[]) {
    ...
    int seconds1 = 600;
    Time time1{9, 30, 0};
    if (fails(time1 + seconds1, "09:40:00") || fails(seconds1 + time1, "09:40:00")) 
    {
        result |= 600; // Assign a unique bit for this test's failure
    } 
    else 
    {
        std::cout << "Test from bonus: " << time1 + seconds1 << " is also " << seconds1 + time1 << std::endl;
    }

    Time time2{9, 30, 0};
    int hour1 = time2[0]; // hour is 9
    int minute1 = time2[1]; // minute is 30
    int second1 = time2[2]; // second is 0

    if (fails(hour1, 9, "Hours")) {result |= 9;}
    if (fails(minute1, 30, "Minutes")){result |= 30;}
    if (fails(second1, 0, "Seconds")){result |= 60;}

    std::cout << "Test from extreme bonus: Hours = " << hour1 <<
    ", Minutes = " << minute1 << ", Seconds = " << second1 << std::endl;

    // -------------------------------------------------------------------------
    // Report overall result (if any failures were detected)
    if(result != 0) {std::cerr << "FAIL with error code " << result << std::endl;}
    else {std::cout << "Looks like all tests inluding our new ones passed." << std::endl;}
