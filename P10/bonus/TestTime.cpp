...
int main(int argc, char* argv[]) {
    ...
    if (fails(time1 + seconds1, "09:40:00") || fails(seconds1 + time1, "09:40:00")) {
        result |= 1 << 6; // Assign a unique bit for this test's failure
    } else {
        std::cout << time1 + seconds1 << " is also " << seconds1 + time1 << std::endl;
    }

    // -------------------------------------------------------------------------
    // Report overall result (if any failures were detected)
    if(result != 0) {std::cerr << "FAIL with error code " << result << std::endl;}
    else {std::cout << "Looks like all tests inluding our new one passed." << std::endl;}
